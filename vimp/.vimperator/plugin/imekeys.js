...
        if (io.run('which', ['python'], true) == 0 &&
	    io.run('which', ['ibus-setup'], true) == 0 &&            io.run('python', [ '-c', 'import ibus' ], true) == 0) {
            return {
                on: function(){
 io.run('python', [ '-c', cmd('enable') ]);
 },                off: function(){
 io.run('python', [ '-c', cmd('disable') ]);
 }            };
        }    })();
...
        if (!liberator.has('Windows') &&            io.run('which', ['xvkbd'], true) == 0) {
            methods.x = {
                send: function(key){
 io.run('xvkbd', [ '-text', key ]);
 }            };
...
                    file = File(file);
                    if (!file.exists()) {
                        file.write([                            'var sh = WScript.CreateObject("WScript.Shell");
...
            key = (typeof k == 'object' && function(m){
 return k[m];
 }) || key;
...
            options.add(                [varname],                'Automatically '+prop+' IME',                'stringlist',                defaultValue,                {
                    getter: function() {
...
                        var completions = [                            [ 'all', 'on all possible elements' ],                            [ 'none', 'on no element' ],                            [ 'cmd', 'on command line' ],                            [ 'insert', 'on input element' ]                        ];
